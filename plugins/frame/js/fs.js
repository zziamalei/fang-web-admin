 ...
 */
layui.config({
  base : "/admin-web/plugins/frame/js/",//设定扩展的Layui模块的所在目录，一般用于外部模块扩展
	version : '1.7.1'
});
			...
			$.each( data, function(k, v) {
				...
				if ( settings.findbyname ) { // by name
					...
					if ( elt.length == 1 ) {
                        ...
                        }else if(elt.attr("dataFormatType")){
                            elt.val( layui.fsUtil.toDateString(v,elt.attr("dataFormatType")));
						}else{
                            elt.val(v);
						}
